...
fn map_values_equal(left: &dyn MapValue, right: &dyn MapValue) -> bool {
...
impl Value {
    fn as_map_value(&self) -> Option<&dyn MapValue> {
        match self {
            Value::Map(map) => Some(map),
            Value::DynamicMap(map) => Some(map.as_ref()),
            _ => None,
        }
    }
    ...
    pub fn type_of(&self) -> ValueType {
        match self {
            ...
            Value::Map(_) | Value::DynamicMap(_) => ValueType::Map,
...
impl PartialEq for Value {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            ...
            (Value::Map(a), Value::DynamicMap(b)) => map_values_equal(a, b.as_ref()),
            (Value::DynamicMap(a), Value::Map(b)) => map_values_equal(a.as_ref(), b),
            (Value::DynamicMap(a), Value::DynamicMap(b)) => map_values_equal(a.as_ref(), b.as_ref()),
...
impl Value {
    ...
    pub fn resolve(expr: &Expression, ctx: &Context) -> ResolveResult {
        match &expr.expr {
                    ...
                    match call.func_name.as_str() {
                        ...
                        operators::IN => {
                            ...
                            match (left, right) {
                                ...
                                (any, map_value)
                                    if matches!(map_value, Value::Map(_) | Value::DynamicMap(_)) =>
                                {
                                    if let Ok(key) = any.try_into() {
                                        let contains = map_value
                                            .as_map_value()
                                            .expect("checked map variant")
                                            .contains_key(&key);
                                        return Value::Bool(contains).into();
                                    } else {
                            ...
                            return match (value, idx) {
                                    ...
                                    if matches!(map_value, Value::Map(_) | Value::DynamicMap(_)) =>
                                {
                                    ...
                                    match index.try_into() {
                                        Ok(key) => map.get(&key).unwrap_or(Value::Null).into(),
                                        Err(index) => {
            ...
            Expr::Select(select) => {
                ...
                if select.test {
                    match left.as_map_value() {
                        Some(map) => Ok(Value::Bool(
                            map.iter().any(|(key, _)| key.to_string() == select.field),
                        )),
            ...
            Expr::Comprehension(comprehension) => {
                ...
                match iter {
                    ...
                    map_value if matches!(map_value, Value::Map(_) | Value::DynamicMap(_)) => {
                        let map = map_value
                            .as_map_value()
                            .expect("checked map variant");
    ...
    fn member(self, name: &str) -> ResolveResult {
        ...
        let child = self
            .as_map_value()
            .and_then(|map| map.get(&name.clone().into()));
