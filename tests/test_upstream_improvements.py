...
class TestOptionalValues:
    ...
    @pytest.mark.xfail(
        reason="Optional values not implemented in cel v0.11.0",
        strict=False  
    )
...
class TestLogicalOperatorBehavior:
    ...
    def test_or_operator_returns_original_values(self):
        ...
        assert result == True, f"Expected True, got {result}"  # Short-circuit works
    ...
    def test_or_operator_expected_cel_spec_behavior(self):
        ...
        assert cel.evaluate('42 || false') == True
        assert cel.evaluate('0 || "default"') == True  
        assert cel.evaluate('false || 0') == False
        assert cel.evaluate('null || false') == False
...
def test_upstream_improvements_summary():
    ...
    improvements_to_watch = {
        "String functions": ["lowerAscii", "upperAscii", "indexOf", "substring", "lastIndexOf", "replace", "split", "join"],
